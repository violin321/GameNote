...
// Play history does not need the account email. Keep the authorization grant
// limited to the identity claims used by Nintendo's own Store client flow.
export const STORE_SCOPE = "openid user user.mii user.links[].id";
...
function assertAllowed(url, method) {
  ...
  try {
    ...
    throw new StoreClientError("store_endpoint_forbidden", 400);
  }
  if (
...
export async function requestStoreJson({ url, method, headers = {}, body, signal }) {
  ...
  if (body && Buffer.byteLength(body) > 32 * 1024)
    throw new StoreClientError("store_request_too_large", 400);
  return new Promise((resolve, reject) => {
    ...
    const req = httpsRequest(
      ...
      (response) => {
        ...
        response.on("data", (chunk) => {
          ...
          if (size > 8 * 1024 * 1024) {
            req.destroy();
            finish(reject, new StoreClientError("store_response_too_large"));
          } else chunks.push(chunk);
        });
        response.on("error", () => finish(reject, new StoreClientError("store_network_error")));
        response.on("end", () => {
          ...
          if (status < 200 || status >= 300) {
            ...
            finish(reject, new StoreClientError(code, status));
            return;
          }
          try {
            ...
            finish(reject, new StoreClientError("store_invalid_upstream_data"));
          }
        });
      },
    );
    const deadline = setTimeout(() => {
      req.destroy();
      finish(reject, new StoreClientError("store_timeout", 504));
    }, 15_000);
    req.on("error", () =>
      finish(
        reject,
        signal?.aborted
          ? new StoreClientError("store_timeout", 504)
          : new StoreClientError("store_network_error"),
      ),
    );
    req.end(body);
  });
...
function secret(value, field) {
  if (typeof value !== "string" || value.length < 8 || value.length > 16_384 || /\s/.test(value))
    throw new StoreClientError(`store_missing_${field}`);
  return value;
}

function nonNegativeInt(value, maximum = Number.MAX_SAFE_INTEGER) {
...
function normalizedHistory(raw) {
  if (!raw || typeof raw !== "object" || !Array.isArray(raw.playHistories))
    throw new StoreClientError("store_invalid_upstream_data");
  if (raw.playHistories.length > 5_000) throw new StoreClientError("store_response_too_large", 502);
  ...
  const playHistories = source
    .map((item) => {
      ...
      const minutes = nonNegativeInt(item.totalPlayedMinutes, maximumSafeMinutes);
  ...
  if (recent.length > 4_000) throw new StoreClientError("store_response_too_large", 502);
  const dates = [];
  const days = [];
  ...
  for (const item of recent) {
    ...
    if (dailyTitleCount > 20_000) throw new StoreClientError("store_response_too_large", 502);
    const dailyPlayHistories = sourceTitles
      .map((title) => {
        ...
        const minutes = nonNegativeInt(title.totalPlayedMinutes, maximumSafeMinutes);
        if (minutes !== undefined) out.totalPlayedMinutes = minutes;
        return Object.keys(out).length ? out : null;
      })
      .filter(Boolean);
