...
export async function initializeDirectory(directory) {
  if (!isAbsolute(directory)) throw new MoonError("absolute_directory_required", 400);
  await privateDirectory(directory, true);
  for (const name of ["secrets", "state", "run"])
    await privateDirectory(join(directory, name), true);
  ...
  return {
    socketPath: join(directory, "run", "sidecar.sock"),
    apiKeyFile: join(directory, "secrets", "api-key"),
  };
}

export async function loadInstallation(directory) {
  if (!isAbsolute(directory)) throw new MoonError("absolute_directory_required", 400);
  await privateDirectory(directory);
  for (const name of ["secrets", "state", "run"]) await privateDirectory(join(directory, name));
  const values = [];
  for (const name of ["api-key", "master-key", "installation-key"]) {
    const text = (await readPrivateFile(join(directory, "secrets", name), 256))
      .toString("utf8")
      .trim();
    if (
