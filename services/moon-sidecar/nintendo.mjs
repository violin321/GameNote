...
// These are public identifiers used by Nintendo's Parental Controls mobile app.
// They are protocol compatibility values, not installation credentials.
export const MOON_CLIENT_ID = "54789befb391a838";
export const MOON_REDIRECT_URI = `npf${MOON_CLIENT_ID}://auth`;
// Nintendo's published mobile-app authorization request includes scopes whose
// names imply writes. This connector still enforces a GET-only Moon endpoint
// allowlist and never calls a parental-control mutation endpoint.
...
export const MOON_DEFAULT_LOCALE = "en-US";
export const MOON_DEFAULT_TIME_ZONE = "UTC";
...
export function moonRequestProfile(environment = process.env) {
  const requestedLocale = environment.MOON_LOCALE?.trim() || MOON_DEFAULT_LOCALE;
  const requestedTimeZone = environment.MOON_TIME_ZONE?.trim() || MOON_DEFAULT_TIME_ZONE;
  let locale;
  try {
    locale = Intl.getCanonicalLocales(requestedLocale)[0];
  } catch {
    throw new MoonError("moon_configuration_invalid", 500);
  }
  if (!locale) throw new MoonError("moon_configuration_invalid", 500);
  try {
    new Intl.DateTimeFormat(locale, { timeZone: requestedTimeZone }).format(0);
  } catch {
    throw new MoonError("moon_configuration_invalid", 500);
  }
  return { locale, timeZone: requestedTimeZone };
}
...
export function createNintendoClient({
  ...
  const requestProfile = moonRequestProfile(environment);
  ...
  async function account(sessionToken, signal) {
    ...
    const user = await call({
      ...
      headers: {
        ...
        "Accept-Language": requestProfile.locale,
      },
    });
  ...
  async function moon(path, accessToken, signal) {
    return call({
      ...
      headers: {
        ...
        "X-Moon-Model": "",
        "X-Moon-TimeZone": requestProfile.timeZone,
        "X-Moon-Os-Language": requestProfile.locale,
        "X-Moon-App-Language": requestProfile.locale,
