...
describe("Moon Unix-socket client", () => {
    ...
    await unixServer(
      (request, response) => {
        ...
        if (request.url === "/v1/status")
          response.end(JSON.stringify({ ...status, session_token: "fixture-sensitive-value" }));
        else if (request.url === "/v1/auth/authorize")
          response.end(
            JSON.stringify({
              authorizationUrl: official,
              expiresAt: Date.now() + 60_000,
              credential: "fixture-sensitive-value",
            }),
          );
      ...
      async (client) => {
        ...
        expect(JSON.stringify(await client.authorize())).not.toContain("fixture-sensitive-value");
  ...
  it("rejects API key files that are not private regular files", async () => {
    const key = randomBytes(32).toString("base64url");
    const file = join(await directory(), "api-key");
    await writeFile(file, `${key}\n`, { mode: 0o600 });
    ...
    await expect(
      readMoonApiKey({ NODE_ENV: "test", MOON_SIDECAR_API_KEY_FILE: file }),
    ).rejects.toMatchObject({ code: "unsafe_private_file" });
    await expect(
      MoonSidecarClient.configured({
        NODE_ENV: "test",
        MOON_SIDECAR_SOCKET_PATH: "/tmp/moon-sidecar.sock",
        MOON_SIDECAR_API_KEY_FILE: file,
      }),
    ).rejects.toMatchObject({ code: "sidecar_not_configured" });

    await chmod(file, 0o600);
    const link = join(await directory(), "api-key-link");
    await symlink(file, link);
    await expect(
      readMoonApiKey({ NODE_ENV: "test", MOON_SIDECAR_API_KEY_FILE: link }),
    ).rejects.toMatchObject({ code: "unsafe_private_file" });

    const oversized = join(await directory(), "api-key-oversized");
    await writeFile(oversized, "x".repeat(257), { mode: 0o600 });
    await expect(
      readMoonApiKey({ NODE_ENV: "test", MOON_SIDECAR_API_KEY_FILE: oversized }),
    ).rejects.toMatchObject({ code: "unsafe_private_file" });
  });
});
