...
describe("Nintendo Store low-frequency scheduler", () => {
  ...
  it("serializes connection changes and makes successful reauthorization immediately due", async () => {
    ...
    await expect(
      withNintendoStoreConnectionChange(async () => "connected", environment),
  ...
  it("requires one explicit absolute private data directory", async () => {
    const { directory, environment } = await fixtureState();
    expect(nintendoStoreSchedulerStateFile(environment)).toBe(join(directory, "scheduler.sqlite"));
    expect(() =>
      nintendoStoreSchedulerStateFile({
        NODE_ENV: "test",
        NINTENDO_STORE_DATA_DIR: "relative-directory",
      }),
    ...
    expect(() => nintendoStoreSchedulerStateFile({ NODE_ENV: "test" })).toThrow("not_configured");
  });
