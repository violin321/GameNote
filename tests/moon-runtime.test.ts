...
describe("Moon fixed network contract", () => {
  it("uses portable locale and time-zone defaults and validates explicit overrides", () => {
    expect(moonRequestProfile({ NODE_ENV: "test" })).toEqual({
      locale: MOON_DEFAULT_LOCALE,
      timeZone: MOON_DEFAULT_TIME_ZONE,
    });
    expect(
      moonRequestProfile({
        NODE_ENV: "test",
        MOON_LOCALE: "zh-cn",
        MOON_TIME_ZONE: "Asia/Tokyo",
      }),
    ).toEqual({ locale: "zh-CN", timeZone: "Asia/Tokyo" });
    expect(() => moonRequestProfile({ NODE_ENV: "test", MOON_LOCALE: "not a locale" })).toThrow(
      "moon_configuration_invalid",
    );
    expect(() => moonRequestProfile({ NODE_ENV: "test", MOON_TIME_ZONE: "not/a-zone" })).toThrow(
      "moon_configuration_invalid",
    );
  });
  ...
  it("collects directly from Nintendo using pinned profile and only retries an idempotent read", async () => {
    ...
    const client = createNintendoClient({
      ...
      transport: async (input: (typeof requests)[number]) => {
        requests.push(input);
        if (input.url.endsWith("/api/token")) return { access_token: "fixture-access-token" };
        if (input.url.endsWith("/users/me")) return { id: "fixture-account" };
    ...
    expect(await client.collect("fixture-session-token", "fixture-account")).toEqual(raw());
    ...
    expect(requests.at(-1)?.headers).toMatchObject({
      ...
      "X-Moon-TimeZone": MOON_DEFAULT_TIME_ZONE,
      "X-Moon-Os-Language": MOON_DEFAULT_LOCALE,
      "X-Moon-App-Language": MOON_DEFAULT_LOCALE,
    });
...
describe("Moon persistent state and synchronization", () => {
  ...
  it("serializes disconnect/authorization with a running sync and sanitizes unexpected failures", async () => {
    ...
    const { runtime } = await linkedRuntime({
      collect: async () => {
        await blocking;
        throw new Error("credential secret and internal endpoint");
      },
    });
  ...
  it("encrypts pending PKCE and credentials with independent 0600 keys and fails closed on key reuse", async () => {
    ...
    const state = {
      version: 1,
      secret: "fixture-session-token",
      pending: { verifier: "fixture-PKCE-verifier" },
    };
  ...
  it("keeps the API key and socket in a separate IPC directory", async () => {
    const privateDirectory = await mkdtemp(join(tmpdir(), "moon-private-"));
    const ipcDirectory = await mkdtemp(join(tmpdir(), "moon-ipc-"));
    directories.push(privateDirectory, ipcDirectory);
    await chmod(privateDirectory, 0o755);
    await chmod(ipcDirectory, 0o755);

    const paths = await initializeDirectory(privateDirectory, { ipcDirectory });
    const installation = await loadInstallation(privateDirectory, { ipcDirectory });

    expect(paths).toEqual({
      socketPath: join(ipcDirectory, "run", "sidecar.sock"),
      apiKeyFile: join(ipcDirectory, "secrets", "api-key"),
    });
    expect(installation.directory).toBe(privateDirectory);
    expect(installation.ipcDirectory).toBe(ipcDirectory);
    await expect(readFile(join(privateDirectory, "secrets", "api-key"))).rejects.toMatchObject({
      code: "ENOENT",
    });
    await expect(readFile(join(ipcDirectory, "secrets", "master-key"))).rejects.toMatchObject({
      code: "ENOENT",
    });
    expect((await stat(join(privateDirectory, "state"))).mode & 0o777).toBe(0o700);
    expect((await stat(join(ipcDirectory, "secrets", "api-key"))).mode & 0o777).toBe(0o600);
  });
  ...
  it("restores valid pending authorization then snapshot and credentials across restarts", async () => {
    ...
    const client = {
      ...
      async account() {
        return { accountId: "fixture-account", accessToken: "fixture-access-token" };
      },
      async collect() {
        return raw();
      },
