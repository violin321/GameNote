...
beforeEach(async () => {
  ...
  process.env.NINTENDO_STORE_PROBE_DIR = join(directory, "credentials");
...
afterEach(async () => {
  ...
  delete process.env.NINTENDO_STORE_PROBE_DIR;
...
describe("Nintendo Store cumulative import", () => {
  it("stores a cumulative total, replaces decreases, and preserves known dates", async () => {
    ...
    withDatabase((db) => {
      ...
      expect(
        db
          .prepare(
            `SELECT s.fetched_at,g.total_seconds,g.play_days FROM nintendo_store_game_snapshots g
             JOIN nintendo_store_sync_snapshots s ON s.id=g.snapshot_id ORDER BY s.fetched_at`,
          )
          .all(),
      ).toEqual([
        { fetched_at: "2026-09-12T05:00:00.000Z", total_seconds: 121_200, play_days: 24 },
        { fetched_at: "2026-09-13T05:00:00.000Z", total_seconds: 1_200, play_days: 2 },
      ]);
      expect(
        db
          .prepare(
            `SELECT official_date,total_seconds,first_seen_at,last_seen_at
  ...
  it("copies a unique Moon association and never adds Moon time to Store time", async () => {
    withDatabase((db) => {
      db.prepare(
        `INSERT INTO purchase_records(
          id,title,title_id,normalized_title,raw_json,imported_at
        ) VALUES('purchase-ac','集合啦！动物森友会','01006f8002326000','集合啦动物森友会','{}','2026-09-01T00:00:00Z')`,
      ).run();
      db.prepare(
        `INSERT INTO play_games(
          ...
          'moon-ac','moon_connector','moon:ac','集合啦！动物森友会','集合啦动物森友会',
          '01006F8002326000','Nintendo Switch','2026-09-01','2026-09-12',44400,12,
      ...
      db.prepare(
        `INSERT INTO play_purchase_links(
          ...
          'link-ac','moon-ac','purchase-ac','confirmed','manual',1,
          '2026-09-12T01:00:00Z','admin','2026-09-12T01:00:00Z','2026-09-12T01:00:00Z'
        )`,
      ).run();
    });
    ...
    const games = await listPlayGames("total", "desc", "动物森友会");
    expect(games).toHaveLength(1);
    expect(games[0]).toMatchObject({
      source: "nintendo_store",
      totalSeconds: 121_200,
      link: { status: "confirmed", purchaseRecordId: "purchase-ac", method: "manual" },
    });
    expect(await listPurchasePlaySummaries()).toEqual([
      expect.objectContaining({ purchaseRecordId: "purchase-ac", totalSeconds: 121_200 }),
    ]);
