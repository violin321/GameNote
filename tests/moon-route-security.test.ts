...
describe("Moon administrator API boundary", () => {
  ...
  it("accepts callback secrets only in a single-field bounded POST body, never query strings", async () => {
    ...
    for (const [path, body, expected] of [
      ["callback?callbackUrl=fixture", '{"callbackUrl":"fixture"}', 400],
      ["callback", '{"callbackUrl":"fixture","extra":true}', 400],
      ...
      expect(JSON.stringify(await response.json())).not.toContain("fixture-sensitive-value");
    }
    expect(client).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    const response = await handler(
      post(path, "https://games.example", JSON.stringify({ fixture: "x".repeat(9000) })),
    );
