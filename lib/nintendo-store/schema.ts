import type { PlayDatabase } from "@/lib/play-history/schema";

/** Store-specific audit tables live beside the generic play-history tables. */
export function ensureNintendoStoreSchema(db: PlayDatabase) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS nintendo_store_game_snapshots (
      snapshot_id TEXT NOT NULL
        REFERENCES nintendo_store_sync_snapshots(id) ON DELETE CASCADE,
      ...
      time_semantics TEXT NOT NULL DEFAULT 'snapshot_observation'
        CHECK(time_semantics = 'snapshot_observation'),
    ...
    CREATE TABLE IF NOT EXISTS nintendo_store_daily_snapshots (
      snapshot_id TEXT NOT NULL
        REFERENCES nintendo_store_sync_snapshots(id) ON DELETE CASCADE,
      ...
      image_url TEXT NOT NULL DEFAULT '',
      total_seconds INTEGER NOT NULL CHECK(total_seconds >= 0),
      time_semantics TEXT NOT NULL DEFAULT 'daily_aggregate'
        CHECK(time_semantics = 'daily_aggregate'),
      PRIMARY KEY(snapshot_id, official_date, external_id)
    );

    CREATE INDEX IF NOT EXISTS idx_nintendo_store_daily_snapshots_identity
      ON nintendo_store_daily_snapshots(official_date, external_id, snapshot_id);
    CREATE INDEX IF NOT EXISTS idx_nintendo_store_daily_snapshots_game
      ON nintendo_store_daily_snapshots(play_game_id, official_date DESC, snapshot_id);
    ...
    CREATE TABLE IF NOT EXISTS nintendo_store_daily_history (
      ...
      last_snapshot_id TEXT NOT NULL
        REFERENCES nintendo_store_sync_snapshots(id),
  ...
  ensureStoreColumn(
    db,
    "nintendo_store_game_snapshots",
    "time_semantics",
    "TEXT NOT NULL DEFAULT 'snapshot_observation' CHECK(time_semantics = 'snapshot_observation')",
  );
  ensureStoreColumn(
    db,
    "nintendo_store_daily_history",
    "time_semantics",
    "TEXT NOT NULL DEFAULT 'daily_aggregate' CHECK(time_semantics = 'daily_aggregate')",
  );
}

function ensureStoreColumn(db: PlayDatabase, table: string, column: string, definition: string) {
