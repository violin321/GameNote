...
export class NintendoStoreScheduleStore {
  ...
  private open() {
    ...
    if (
      ...
      throw new NintendoStoreError("store_data_directory_unsafe", 503);
  ...
  finish(owner: string, now: number, error: string | null) {
    ...
    try {
      ...
      const delay = error
        ? error === "store_reauthorization_required" || error === "store_credential_invalid"
...
export function nintendoStoreSchedulerStateFile(environment: NodeJS.ProcessEnv = process.env) {
  return join(nintendoStoreDataDirectory(environment), "scheduler.sqlite");
}
...
export async function syncNintendoStoreScheduled(
  ...
  if (options.automatic) {
    ...
    if (credentialStatus === "not_connected") return null;
  }
...
export function readNintendoStoreScheduleStatus(environment: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    ...
    reauthorizationRequired:
      row?.last_error === "store_reauthorization_required" ||
      row?.last_error === "store_credential_invalid",
  };
}
