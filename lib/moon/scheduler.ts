...
export function moonSchedulerStateFile(environment: NodeJS.ProcessEnv = process.env) {
  ...
  const configuredDatabase =
    environment.APP_DATABASE_FILE?.trim() || environment.SWITCH_LEDGER_DATABASE_FILE?.trim();
  const databaseFile = configuredDatabase
    ? isAbsolute(configuredDatabase)
      ? configuredDatabase
      : resolve(process.cwd(), configuredDatabase)
    : resolve(process.cwd(), "data", "records.sqlite");
  return resolve(dirname(databaseFile), "moon-scheduler.sqlite");
}
