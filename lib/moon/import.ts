...
import {
  createSuggestedPurchaseLink,
  openDatabase,
  type DatabaseSync,
} from "../play-history/repository";
  ...
  try {
    ...
    try {
      ...
      for (const gameId of affected) {
        // No play_sessions or play_observations are written for daily totals.
        db.prepare(
        ...
        createSuggestedPurchaseLink(db, gameId, now);
      }
...
function upsertMoonGame(
  ...
  if (!previous) {
    ...
    db.prepare(
      `INSERT INTO play_games(id,source,external_id,title,normalized_title,title_id,official_url,
      first_played_at,last_played_at,created_at,updated_at) VALUES(?,'moon_connector',?,?,?,?,?,'','',?,?)`,
    ).run(
      id,
      externalId,
      game.title,
  ...
  ensurePlayGameEntityBinding(db, id, now);
