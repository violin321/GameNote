...
/** Moon-specific audit tables are installed lazily by the Moon importer/status reader. */
export function ensureMoonSchema(db: PlayDatabase) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS moon_connector_devices (
      account_scope TEXT NOT NULL
        REFERENCES moon_connector_accounts(account_scope) ON DELETE CASCADE,
    ...
    CREATE TABLE IF NOT EXISTS moon_connector_reports (
      ...
      result TEXT NOT NULL
        CHECK(result IN ('CALCULATING','ACHIEVED','UNACHIEVED','UNKNOWN')),
      ...
      FOREIGN KEY(account_scope, device_id)
        REFERENCES moon_connector_devices(account_scope, device_id) ON DELETE CASCADE
    );

    CREATE INDEX IF NOT EXISTS idx_moon_connector_reports_date
      ON moon_connector_reports(official_date DESC);
    ...
    CREATE INDEX IF NOT EXISTS idx_moon_connector_report_games_game
      ON moon_connector_report_games(play_game_id);
  `);
}
