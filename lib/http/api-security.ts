...
/**
 * Browser writes must carry an exact same-origin Origin header. Direct API
 * clients normally omit Origin and must opt in with X-GameNote-API: 1.
 */
export function assertSameOriginWrite(request: NextRequest) {
  ...
  if (origin) {
    ...
    if (parsed.origin !== `${protocol}://${host}`) {
      throw new ApiRequestError(403, "禁止跨源写入");
    }
    return;
  }
  ...
  if (fetchSite || !explicitApiClient) {
    throw new ApiRequestError(403, "写请求需要同源 Origin");
  }
}
