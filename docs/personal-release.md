...
`docker-compose.yml` 要求显式设置 `GAMENOTE_IMAGE=<自己的命名空间>/gamenote@sha256:<经验证的摘要>`，避免误用不含 NS2 功能的上游镜像；不要把 NS2 数据库交给上游镜像。单镜像内的 Moon 和 Store 状态均在 `/data`，见 `docs/container-runtime.md`。升级前用 `docs/ns2-backup.md` 做完整在线数据库备份、校验和隔离恢复演练；Moon/Store 授权状态与密钥也必须单独保管。先在隔离环境验证数据迁移、页面与授权状态，再考虑单实例生产切换和回滚，不在发布工作流中自动部署 OVH。
